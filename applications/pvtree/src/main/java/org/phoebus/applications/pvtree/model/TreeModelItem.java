...
public class TreeModelItem
{
    ...
    private void fetchType()
    {
        try
        {
            ...
            type_flow = pv.onValueEvent(BackpressureStrategy.LATEST).firstOrError().subscribe(value ->
            {
    ...
    private void resolveNextLink()
    {
        ...
        try
        {
            ...
            link_flow = pv.onValueEvent(BackpressureStrategy.LATEST).firstOrError().subscribe(link_consumer);
            link_pv.set(pv);
        }
        catch (Exception ex)
        {
